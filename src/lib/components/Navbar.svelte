...
>
	<div class="nav-container mx-auto flex h-16 max-w-screen-xl items-center justify-between px-4">
		...
		<!-- Right side container for mobile controls and desktop links -->
		<div class="flex items-center gap-2">
			...
			<div class="flex items-center md:hidden">
				...
				<button
					...
					on:click={() => {
						toggleTheme();
						// No need to close menu here as it's outside the menu
					}}
				...
				<button
					class="menu-toggle btn btn-ghost btn-square"
					on:click={toggleMenu}
			...
			<!-- Navigation Links & Theme Toggle (Desktop) -->
			<div
				class="nav-links bg-base-100/95 md:gap-auto border-base-300/50 absolute top-16 right-0 left-0
                 ...
                 md:items-center md:gap-8 md:border-none md:bg-transparent md:py-0 md:opacity-100 md:backdrop-blur-none {isMenuOpen
					? 'opacity-100'
			...
			>
				...
				<button
					class="theme-toggle btn btn-ghost btn-circle hidden md:inline-flex"
					on:click={() => {
						toggleTheme();
						closeMenu(); // Keep closeMenu here for desktop dropdown if ever implemented differently
					}}
					aria-label="Toggle theme"
				>
					{#if $theme === 'dark'}
